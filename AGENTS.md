Concise rules for building maintainable software with accessible, fast, delightful UIs.
Use MUST/SHOULD/NEVER to guide decisions.

## GENERAL

- MUST: Follow **KISS** religiously. Do not overcomplicate things unless necessary.
- MUST: Optimize for shipping the requested requirements first.
- SHOULD: Occasionally refactor your work. Keep code/components organized logically, and name things clearly.
- MUST: Prefer boring, well-known patterns over cleverness.
- NEVER: Add “nice-to-have” features, abstractions, or architecture unless it’s required by the stated goals/constraints.

## BACKEND

### Testing policy

- MUST: Test your work with tests.
- SHOULD: Integration tests are usually sufficient.
- MUST: For critical components (e.g. DB with user data/transactions, authentication/authorization, money/time-sensitive logic), add **component tests** (and/or narrower tests) to cover edge cases and failure modes.
- MUST: Use **TDD** for backend work: write/extend a failing test first for the behavior, then implement, then refactor.

### Engineering

- MUST: Validate inputs at boundaries; return clear errors.
- MUST: Make side effects explicit (DB writes, external calls).
- SHOULD: Keep handlers thin; put domain logic in testable modules.
- NEVER: Log secrets/credentials; treat user data carefully.

## FRONTEND

## Iterative UI improvement loop ("have eyes")

When building UI/UX-heavy work, follow this loop until satisfied:

1) Implement a small change
2) View the change (run/render/snapshot)
3) Evaluate visually + UX-wise (layout, motion, readability, affordances, input feel)
4) Adjust
...
- MUST: Test critical user flows (the ones that would anger users if broken).
- SHOULD: Prefer a small number of high-value integration/e2e tests over lots of brittle micro-tests.
- SHOULD: Test pure UI logic/state transitions with unit tests when it’s cheaper than e2e.
- MUST: Manually verify keyboard navigation + focus states for interactive components.
